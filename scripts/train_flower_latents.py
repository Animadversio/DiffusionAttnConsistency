...
class ImageLatentsDataset(Dataset):
    filename = "flower_latents10k_pilots.pt"
    ...
    def __init__(self, transform=None):
        ...
        self.data = torch.load(join(self.savedir, self.filename))
        print("data loaded from ", join(self.savedir, self.filename))
...
def main(args):
    ...
    dataset = ImageLatentsDataset(transform=None)
    ...
    for epoch in range(args.epochs):
        ...
        for x in loader:
            x = x.to(device) # this is already mapped to the latents
            x = x * 0.18215
            ...
            if train_steps % args.save_samples_every == 0 or \
                ...
                if rank == 0:
                    ...
                    writer.add_images(f"samples/{train_steps:07d}", samples, train_steps)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--results-dir", type=str, default="/n/holylfs06/LABS/kempner_fellow_binxuwang/Users/binxuwang/DL_Projects/DiffusionAttnConsistency/DiT/flowers_results")
    parser.add_argument("--model", type=str, choices=list(DiT_models.keys()), default="DiT-S/2")
    parser.add_argument("--image-size", type=int, choices=[16, 64, 128], default=16)
    parser.add_argument("--dataset", type=str, default="flower_latents10k_pilots")
